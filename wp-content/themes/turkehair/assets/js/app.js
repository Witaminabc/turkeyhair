...
/******/ 	return __webpack_require__(__webpack_require__.s = "./src/assets/scripts/main2.js");
/******/ })
/************************************************************************/
/******/ ({
...
jquery__WEBPACK_IMPORTED_MODULE_1___default()(document).ready(function () {
  class BeforeAfterSlider {
    constructor(container) {
      this.container = jquery__WEBPACK_IMPORTED_MODULE_1___default()(container);
      this.beforeImage = this.container.find('.image-before');
      this.sliderHandle = this.container.find('.slider-handle');
    ...
    bindEvents() {
      // События мыши для конкретного контейнера
      this.container.on('mousedown', this.startDrag.bind(this));
      this.sliderHandle.on('mousedown', this.startDrag.bind(this)); // Клик по кнопке ползунка - предотвращаем всплытие

      this.sliderHandle.on('mousedown touchstart', function (e) {
        e.stopPropagation();
      });
    }
    ...
    startDrag(e) {
      ...
      this.container.addClass('dragging'); // Блокировка выделения текста

      jquery__WEBPACK_IMPORTED_MODULE_1___default()('body').css({
        'user-select': 'none',
        'cursor': 'col-resize'
      }); // Привязываем события к document для плавного перемещения

      jquery__WEBPACK_IMPORTED_MODULE_1___default()(document).on('mousemove.beforeAfter', this.onDrag.bind(this));
      jquery__WEBPACK_IMPORTED_MODULE_1___default()(document).on('mouseup.beforeAfter', this.stopDrag.bind(this)); // События касания для мобильных устройств

      jquery__WEBPACK_IMPORTED_MODULE_1___default()(document).on('touchmove.beforeAfter', this.onDrag.bind(this));
      jquery__WEBPACK_IMPORTED_MODULE_1___default()(document).on('touchend.beforeAfter', this.stopDrag.bind(this));
    }
    ...
    onDrag(e) {
      ...
      const percentage = x / containerRect.width * 100; // Ограничиваем в пределах 0-100%
    ...
    stopDrag() {
      if (!this.isDragging) return;
      this.isDragging = false;
      this.container.removeClass('dragging'); // Разблокировка выделения текста
      ...
      jquery__WEBPACK_IMPORTED_MODULE_1___default()(document).off('.beforeAfter');
    }

    updateSliderPosition(percentage) {
      // Обновляем ширину изображения "до"
      this.beforeImage.css('width', percentage + '%'); // Обновляем позицию ползунка

      this.sliderHandle.css('left', percentage + '%');
    }

    setInitialPosition() {
      this.updateSliderPosition(50); // Начальная позиция 50%
  ...
  const sliders = [];
  jquery__WEBPACK_IMPORTED_MODULE_1___default()('.before-after').each(function (index) {
    const slider = new BeforeAfterSlider(this);
    sliders.push(slider); // Добавляем data-атрибут для идентификации

    jquery__WEBPACK_IMPORTED_MODULE_1___default()(this).attr('data-slider-id', index);
  }); // Глобальное управление событиями мыши (для плавности)

  jquery__WEBPACK_IMPORTED_MODULE_1___default()(document).on('mouseup touchend', function () {
    jquery__WEBPACK_IMPORTED_MODULE_1___default()('body').css({
      'user-select': '',
      'cursor': ''
    });
    jquery__WEBPACK_IMPORTED_MODULE_1___default()(document).off('.beforeAfter');
  }); // Управление через кнопки (если есть)

  jquery__WEBPACK_IMPORTED_MODULE_1___default()('.control-btn').on('click', function () {
    const action = jquery__WEBPACK_IMPORTED_MODULE_1___default()(this).data('action');
    const sliderId = jquery__WEBPACK_IMPORTED_MODULE_1___default()(this).data('slider') || 0; // По умолчанию первый слайдер

    if (sliders[sliderId]) {
      switch (action) {
        case 'before':
          sliders[sliderId].showBefore();
          break;

        case 'after':
          sliders[sliderId].showAfter();
          break;

        case 'reset':
          sliders[sliderId].reset();
          break;
      }
    }
  }); // Автоматическое управление для каждого слайдера

  jquery__WEBPACK_IMPORTED_MODULE_1___default()('.slider-controls .control-btn').on('click', function () {
    const $parent = jquery__WEBPACK_IMPORTED_MODULE_1___default()(this).closest('.before-after-container');
    const sliderId = $parent.index('.before-after-container');
    const action = jquery__WEBPACK_IMPORTED_MODULE_1___default()(this).data('action');
...
jquery__WEBPACK_IMPORTED_MODULE_1___default()(document).ready(function () {
  ...
  const videoSwiper = new swiper_bundle__WEBPACK_IMPORTED_MODULE_0__["default"]('.video-slider', {
    slidesPerView: 1,
    spaceBetween: 20,
    loop: true,
    speed: 600,
  ...
  jquery__WEBPACK_IMPORTED_MODULE_1___default()('.video-link').on('click', function (e) {
    e.preventDefault();
    const videoUrl = jquery__WEBPACK_IMPORTED_MODULE_1___default()(this).attr('href');
    const videoId = jquery__WEBPACK_IMPORTED_MODULE_1___default()(this).data('video-id'); // Вставляем видео в модальное окно

    jquery__WEBPACK_IMPORTED_MODULE_1___default()('#videoPlayer').html(`<iframe src="${videoUrl}?autoplay=1&rel=0" 
                    allow="autoplay; encrypted-media" 
                    allowfullscreen></iframe>`); // Показываем модальное окно

    jquery__WEBPACK_IMPORTED_MODULE_1___default()('#videoModal').fadeIn(300); // Останавливаем слайдер при открытии модального окна
  ...
  jquery__WEBPACK_IMPORTED_MODULE_1___default()(window).on('click', function (e) {
    if (jquery__WEBPACK_IMPORTED_MODULE_1___default()(e.target).is('#videoModal')) {
      closeVideoModal();
    }
  }); // Закрытие по ESC
  ...
  function closeVideoModal() {
    jquery__WEBPACK_IMPORTED_MODULE_1___default()('#videoModal').fadeOut(300);
    jquery__WEBPACK_IMPORTED_MODULE_1___default()('#videoPlayer').empty(); // Возобновляем слайдер

    videoSwiper.autoplay.start();
  } // Ресайз окна - обновление Swiper
...
jquery__WEBPACK_IMPORTED_MODULE_1___default()(document).ready(function () {
  // Инициализация Swiper
  const videoSwiper = new swiper_bundle__WEBPACK_IMPORTED_MODULE_0__["default"]('.licensia-slider', {
    slidesPerView: 1,
    spaceBetween: 20,
    loop: true,
    speed: 600,
